...
describe("VALIDATOR", () => {
	const belfioreCodeMatcher = new RegExp(
		`^(?:${BELFIORE_CODE_MATCHER})$`,
		"giu"
	);

	describe("Belfiore Codes", function () {
		this.timeout(5000);
		it("Should validate all codes", async () => {
			const places = await codiceFiscaleUtils.belfioreConnector.toArray();
			const belfioreCodes = places?.map(({ belfioreCode }) => belfioreCode);
			belfioreCodes.every(
				(belfioreCode) =>
					(belfioreCode.match(belfioreCodeMatcher) || [])[0] === belfioreCode
			).should.be.true;
		});
	});
	...
	describe("Omocode", () => {
		...
		describe("OMOCODE_NON_ZERO_NUMBER_LIST", () => {
			const omocodeMatcher = new RegExp(
				`^[${OMOCODE_NON_ZERO_NUMBER_LIST}]$`,
				"i"
			);
	...
	describe("Partial CF", () => {
		describe("PARTIAL_CF_FULL_NAME", () => {
			const partialFullNameMatcher = new RegExp(
				`^${PARTIAL_CF_FULL_NAME}$`,
				"i"
			);
