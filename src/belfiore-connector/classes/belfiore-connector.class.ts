...
export default class BelfioreConnector {
	...
	public static get(
		...
		if (
			...
			(paramName === "byProvince" &&
				((resource.codeMatcher instanceof RegExp &&
					resource.codeMatcher.test("Z000")) ||
					resource.province))
		) {
			return;
		}
	...
	constructor({
		...
		return new Proxy<BelfioreConnector>(
			this,
			this.constructor as typeof BelfioreConnector
		);
	}
	...
	private locationByIndex(
		...
		const belFioreInt = parseInt(
			resourceData.belfioreCode.substr(belfioreIndex, 3),
			32
		);
		...
		const creationDate = BelfioreConnector.decodeDate(
			(resourceData.creationDate || "").substr(dateIndex, 4) || "0"
		).startOf("day");
		const expirationDate = BelfioreConnector.decodeDate(
			(resourceData.expirationDate || "").substr(dateIndex, 4) || "2qn13"
		).endOf("day");
		if (
			(this.fromDate &&
